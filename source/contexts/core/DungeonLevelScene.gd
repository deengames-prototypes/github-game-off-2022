...
func _ready():
	_input_system.connect("move_player", _player_controller, "on_move_player")
	...
	for tile_id in _entities_tile_map.tile_set.get_tiles_ids():
		_entities_tile_id[_entities_tile_map.tile_set.tile_get_name(tile_id)] = tile_id

	_player.tile_position = Vector2(3, 7)
	_entities_tile_map.set_cellv(_player.tile_position, _entities_tile_id["player"])

func on_player_moved(old: Vector2, new: Vector2):
	_entities_tile_map.set_cellv(old, -1)
	_entities_tile_map.set_cellv(new, _entities_tile_id["player"])
